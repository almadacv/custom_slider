			...
			?>
					<div class="slider_elemento">
						<a href="<?php the_permalink() ?> ">
							<div class="slider_img"><?php the_post_thumbnail('post-thumbnail', array('class' => '.elementor-post__thumbnail')) ?> </div>
							<!--<div class="slider_img"><? //php the_post_thumbnail('post-thumbnail', array('class' => '.elementor-post__thumbnail, .elementor-fit-height, .lazyloaded')) 
														?> </div>-->
	...
	<script>
		...
		$jq('.slider_gera').each(function(index) {
			...
			$jq(this).not('.slick-initialized').slick({
				...
				//			prevArrow: '<div class="slider-prev fa fa-angle-left fa-2x"></div>',
				//			nextArrow: '<div class="slider-next fa fa-angle-right fa-2x"></div>',
				responsive: [{
						breakpoint: 1024,
						settings: {
							slidesToShow: 3,
